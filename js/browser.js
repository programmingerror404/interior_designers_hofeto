$(function() {
	...
	var foldersearch = window.location.pathname.split('/').pop().split('.')[0];
	// Start by fetching the file data from scan.php with an AJAX request

	$.get('scan.php?v=' + foldersearch, function(data) {
		var response = [data],
			currentPath = '',
			breadcrumbsUrls = [];
		...
		function render(data) {
			...
			if (scannedFolders.length) {
				...
				scannedFolders.forEach(function(f) {
					...
					if (imageExists(iconpath)) {
						icon = '<div style="margin: auto;position: relative;top: 50%;transform: translateY(-70%);width: 60%;height: 60%;background-position: center center;background-size: cover; background-repeat:no-repeat;background-image: url(\'' + iconpath + '\');"></div>';
					} else if (itemsLength) {
			...
			if (scannedFiles.length) {
				...
				scannedFiles.forEach(function(f) {
					...
					if (fileType == "jpg") {
						icon = '<div style="width: 100%;height: 100%;background-position: center center;background-size: cover; background-repeat:no-repeat;background-image: url(\'' + f.path + '\');"></div>';
